...
ROOMS = [
    {"id": "MARIJA", "name": "Soba MARIJA (standardna)", "capacity": 2},
    {"id": "TINKARA", "name": "Soba TINKARA (standardna)", "capacity": 2},
    {"id": "CILKA", "name": "Soba CILKA (standardna)", "capacity": 2},
    {"id": "HANA", "name": "Soba HANA (standardna)", "capacity": 2},
    {"id": "MANCA", "name": "Soba MANCA (dostopna za invalide)", "capacity": 2},
    {"id": "URSKA", "name": "Apartma URŠKA (družinska, čajna kuhinja)", "capacity": 4},
    {"id": "ANA", "name": "Apartma ANA (družinska, čajna kuhinja)", "capacity": 4},
]
ROOM_NAME_MAP = {
    "marija": "MARIJA",
    "tinkara": "TINKARA",
...
class ReservationService:
    ...
    def get_recent_conversations(self, hours: int = 1, since: str | None = None) -> list[dict]:
        ...
        try:
            cur = conn.cursor()
            if since:
                # Uporabi eksplicitni cutoff (format "YYYY-MM-DD HH:MM:SS")
                cutoff = since
            else:
                import os
                tz_offset = int(os.environ.get("TZ_OFFSET_HOURS", "0"))
                cutoff = (datetime.now() + timedelta(hours=tz_offset) - timedelta(hours=hours)).strftime("%Y-%m-%d %H:%M:%S")
            sql = f"""
    ...
    def get_recent_sessions(self, hours: int = 1, since: str | None = None) -> list[dict]:
        ...
        # Pretvori v seznam, sortiraj po času zadnjega sporočila
    ...
    def get_last_report_time(self, report_type: str = "daily") -> str | None:
        ...
        try:
            cur = conn.cursor()
            cur.execute(f"SELECT sent_at FROM report_log WHERE report_type = {ph} ORDER BY sent_at DESC LIMIT 1", (report_type,))
            row = cur.fetchone()
    ...
    def save_report_time(self, report_type: str = "daily") -> None:
        ...
        try:
            ...
            cur.execute(f"INSERT INTO report_log (report_type, sent_at) VALUES ({ph}, {ph})", (report_type, now))
            conn.commit()
        except Exception:
            pass
        finally:
