...
def send_admin_notification_email(subject: str, html_content: str) -> bool:
    """
    Pošlje splošno admin obvestilo (npr. nove pogovore).
    Pošlje SAMO na CC_ADMIN_EMAIL (ne na info@kovacnik.com).

    Args:
        subject: Zadeva emaila
        html_content: HTML vsebina (brez wrapperja - bo dodan avtomatsko)

    Returns:
        True če uspešno poslano
    """
    if not CC_ADMIN_EMAIL:
        print("[EMAIL] CC_ADMIN_EMAIL ni nastavljen - obvestilo ni poslano")
        return False

    # Dodaj SUBJECT_PREFIX če je nastavljen
    if SUBJECT_PREFIX:
        subject = f"{SUBJECT_PREFIX} {subject}"

    html = _email_wrapper(html_content)
    return _send_email(CC_ADMIN_EMAIL, subject, html)
